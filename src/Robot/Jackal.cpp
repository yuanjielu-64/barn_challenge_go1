...
void Robot_config::robotStatusCallback(const nav_msgs::Odometry::ConstPtr &msg) {
    ...
    if (can_move && !global_goal_odom.empty()) {
        double dist = l2_distance(robot_state.x_, robot_state.y_, global_goal_odom[0], global_goal_odom[1]);
        if (dist <= 1)
            can_move = false;
    }
}
...
void Robot_config::arrayCallback(const std_msgs::Float64MultiArray::ConstPtr& msg) {
    // timeInterval.clear();
    //
...
bool Robot_config::setup() {
    ...
    if (getRobotState() != INITIALIZING && getPoseState().valid_ && can_move ) {
        return true;
    }
...
Robot_config::Robot_config()
    ...
    robot_pose_sub = nh.subscribe("/odom", 10, &Robot_config::robotStatusCallback, this);
    laser_scan_sub = nh.subscribe("/go1_gazebo/scan", 10, &Robot_config::laserScanCallback, this);
    ...
    // array_dt_sub = nh.subscribe("/array_dt", 1, &Robot_config::arrayCallback, this);
